...
const normalizeEvent = function (apiEvent, now, roomId) {
  ...
  event.summary = event.summary || 'Private event'
...
const getSchedule = function (room, now, cb) {
  ...
  getEvents(room, now, (err, response) => {
    if (err || !response) { cb(new Error(true), null); return }
...
const bookEvent = function (roomSlug, booking, cb) {
  ...
  calendar.events.insert({
    ...
    if (err) { return cb(new Error('Couldn\'t create the event'), null) }
    let event = normalizeEvent(response, now)
    cb(null, event)
  })
}

const removeEvent = function (roomSlug, eventId, cb) {
  let room = rooms[roomSlug]
  if (!room) { cb(new Error('Room does not exist'), null); return }
  let params = {
    calendarId: room.id,
  ...
  calendar.events.delete(params, function (err, response) {
    if (err) { return cb(new Error('Couldn\'t delete the event'), null) }
    let res = response
    if (!response) { res = 'Event removed succesfully' }
    cb(null, res)
  })
}
